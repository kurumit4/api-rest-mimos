import { useState, useEffect } from "react";

const API = import.meta.env.VITE_API_URL || "http://localhost:8081";

function getHeaders() {
  const token = localStorage.getItem("token");
  return {
    "Content-Type": "application/json",
    ...(token ? { Authorization: `Bearer ${token}` } : {}),
  };
}

const FORM_INICIAL = { usuarioId: "" };

export default function GestionCarritos() {
  const [carritos, setCarritos] = useState([]);
  const [form, setForm]         = useState(FORM_INICIAL);
  const [cargando, setCargando] = useState(false);
  const [creando, setCreando]   = useState(false);
  const [error, setError]       = useState("");

  useEffect(() => { cargarCarritos(); }, []);

  async function cargarCarritos() {
    setCargando(true);
    setError("");
    try {
      const res = await fetch(`${API}/api/carritos`, { headers: getHeaders() });
      if (!res.ok) throw new Error(`Error ${res.status}`);
      setCarritos(await res.json());
    } catch (e) {
      setError("No se pudo cargar los carritos: " + e.message);
    } finally {
      setCargando(false);
    }
  }

  async function handleCrear(e) {
    e.preventDefault();
    setCreando(true);
    setError("");
    try {
      const res = await fetch(`${API}/api/carritos`, {
        method: "POST",
        headers: getHeaders(),
        body: JSON.stringify({ usuarioId: parseInt(form.usuarioId, 10) }),
      });
      if (!res.ok) throw new Error(`Error ${res.status}`);
      setForm(FORM_INICIAL);
      await cargarCarritos();
    } catch (e) {
      setError("No se pudo crear el carrito: " + e.message);
    } finally {
      setCreando(false);
    }
  }

  async function handleEliminar(id) {
    if (!window.confirm("¿Seguro que deseas eliminar este carrito?")) return;
    setError("");
    try {
      const res = await fetch(`${API}/api/carritos/${id}`, {
        method: "DELETE",
        headers: getHeaders(),
      });
      if (!res.ok) throw new Error(`Error ${res.status}`);
      await cargarCarritos();
    } catch (e) {
      setError("No se pudo eliminar el carrito: " + e.message);
    }
  }

  return (
    <div className="min-h-screen bg-[var(--color-fondo)] p-6">
      <h1 className="text-2xl font-bold text-[var(--color-texto)] mb-6">Gestión de Carritos</h1>

      {error && (
        <div className="mb-4 p-3 rounded-lg bg-red-100 border border-red-300 text-red-700 text-sm">{error}</div>
      )}

      <div className="bg-[var(--color-superficie)] border border-[var(--color-borde)] rounded-xl p-5 mb-8 shadow-sm">
        <h2 className="text-lg font-semibold text-[var(--color-texto)] mb-4">Nuevo Carrito</h2>
        <form onSubmit={handleCrear} className="flex flex-col gap-3 sm:flex-row sm:items-end">
          <div className="flex-1">
            <label className="block text-xs text-[var(--color-texto-suave)] mb-1">ID de Usuario *</label>
            <input
              type="number"
              required
              value={form.usuarioId}
              onChange={(e) => setForm({ usuarioId: e.target.value })}
              placeholder="1"
              className="w-full rounded-lg border border-[var(--color-borde)] bg-[var(--color-fondo)] text-[var(--color-texto)] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-acento)]"
            />
          </div>
          <button
            type="submit"
            disabled={creando}
            className="bg-[var(--color-acento)] text-white font-semibold px-5 py-2 rounded-lg text-sm hover:opacity-90 disabled:opacity-50 transition-opacity whitespace-nowrap"
          >
            {creando ? "Creando…" : "+ Crear Carrito"}
          </button>
        </form>
      </div>

      <div className="bg-[var(--color-superficie)] border border-[var(--color-borde)] rounded-xl shadow-sm overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[var(--color-borde)] bg-[var(--color-secundario-soft)]">
              <th className="text-left px-4 py-3 text-[var(--color-texto-suave)] font-semibold">ID Carrito</th>
              <th className="text-left px-4 py-3 text-[var(--color-texto-suave)] font-semibold">ID Usuario</th>
              <th className="text-left px-4 py-3 text-[var(--color-texto-suave)] font-semibold">Creado el</th>
              <th className="text-center px-4 py-3 text-[var(--color-texto-suave)] font-semibold">Acciones</th>
            </tr>
          </thead>
          <tbody>
            {cargando ? (
              <tr><td colSpan={4} className="text-center py-10 text-[var(--color-texto-suave)]">Cargando…</td></tr>
            ) : carritos.length === 0 ? (
              <tr><td colSpan={4} className="text-center py-10 text-[var(--color-texto-suave)]">No hay carritos registrados.</td></tr>
            ) : (
              carritos.map((c) => (
                <tr key={c.idCarrito} className="border-b border-[var(--color-borde)] hover:bg-[var(--color-secundario-soft)] transition-colors">
                  <td className="px-4 py-3 text-[var(--color-texto-suave)]">{c.idCarrito}</td>
                  <td className="px-4 py-3 text-[var(--color-texto)]">{c.usuarioId}</td>
                  <td className="px-4 py-3 text-[var(--color-texto-suave)]">
                    {c.creadoAt ? new Date(c.creadoAt).toLocaleString("es-CO") : "—"}
                  </td>
                  <td className="px-4 py-3 text-center">
                    <button
                      onClick={() => handleEliminar(c.idCarrito)}
                      className="text-red-600 border border-red-300 px-3 py-1 rounded-md text-xs font-semibold hover:bg-red-600 hover:text-white transition-colors"
                    >
                      Eliminar
                    </button>
                  </td>
                </tr>
              ))
            )}
          </tbody>
        </table>
      </div>
    </div>
  );
}
